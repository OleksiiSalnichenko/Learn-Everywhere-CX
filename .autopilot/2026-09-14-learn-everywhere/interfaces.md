...
Ticket 05: `PlaybackController.play(defaultDictionaryId)`, `pause()`, `resume()`, `stop()`, `retry()` і `observeState(): StateFlow<PlaybackUiState>` керують `PlaybackService`. `PlaybackUiState` має `Stopped`, `Preparing`, `Active(word, phase, index, total, paused, showCard)` та `Error`. `PlaybackService` є `MediaSessionService`; `PlaybackQueue.create(...)` формує immutable session snapshot, `PlaybackBatchPreparer.prepare(...)` готує bounded window, а `AndroidAudioPreparer.prepare(event)` синтезує TTS/WAV-тишину в atomic active-safe cache до 100 MB.
